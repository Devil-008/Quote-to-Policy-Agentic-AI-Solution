...
function KnowledgeBase() {
  ...
  const load = () => {
    api.get('/rag/documents').then(r => setDocs(r.data.documents)).finally(() => setL(false))
  }
  useEffect(load, [])
  ...
  const upload = async () => {
    ...
    try {
      ...
      setOk('Document indexed successfully'); setFile(null); setTitle('')
      load()
  ...
  return (
    <div>
      ...
      <Card className="mb-5">
        <p className="text-sm font-semibold mb-3">Upload Document (PDF / DOCX / TXT)</p>
        {err && <Alert type="error" message={err} />}
        {ok  && <Alert type="success" message={ok} />}
        <div className="flex gap-3 items-end flex-wrap">
          <Input label="Title" value={title} onChange={setTitle} placeholder="Document title…" className="flex-1 min-w-48" />
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-[#6b7280]">File</label>
            <input type="file" accept=".pdf,.docx,.txt"
              onChange={e => setFile(e.target.files[0])}
              className="text-sm text-[#6b7280] file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:bg-[#6366f1] file:text-white file:text-xs file:font-semibold cursor-pointer" />
          </div>
          <Btn onClick={upload} disabled={uploading || !file || !title}>
...
function RAGChat() {
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages(m => [...m, { role:'assistant', content:'Error: ' + (e.response?.data?.detail || 'Failed') }])
    } finally { setL(false) }
  }

  return (
    <div>
      <SectionHeader title="RAG Knowledge Chat" subtitle="Ask questions from indexed documents using local Mistral LLM" />
      <Card className="flex flex-col" style={{ height:'65vh' }}>
        <div className="flex-1 overflow-y-auto space-y-4 mb-4">
          {messages.length === 0 && (
            <p className="text-center text-[#6b7280] text-sm mt-10">Ask a question about your insurance knowledge base…</p>
          )}
          {messages.map((m, i) => (
              ...
              <div className={`max-w-[75%] rounded-xl px-4 py-3 text-sm ${
                ...
                {m.sources?.length > 0 && (
                  <div className="mt-2 pt-2 border-t border-[#2a2f45]">
                    ...
                    {m.sources.map((s, j) => (
                      <p key={j} className="text-xs text-[#2dd4bf]">{s.title} ({(s.score * 100).toFixed(0)}%)</p>
                    ))}
                  </div>
                )}
              </div>
