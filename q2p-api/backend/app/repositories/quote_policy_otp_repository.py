...
from ..models.otp import OTPRecord, OTPStatus, ConsentRecord, EscalationLog, NotificationLog, AuditLog
...
class PolicyRepository:
    ...
    async def get_by_customer(self, customer_id: str) -> List[Policy]:
        result = await self.db.execute(
            select(Policy).where(Policy.customer_id == customer_id).order_by(Policy.created_at.desc())
        )
        return result.scalars().all()

    async def list_all(self, skip: int = 0, limit: int = 50) -> List[Policy]:
        result = await self.db.execute(select(Policy).order_by(Policy.created_at.desc()).offset(skip).limit(limit))
        return result.scalars().all()

    async def update(self, policy_id: str, data: dict) -> Optional[Policy]:
        await self.db.execute(update(Policy).where(Policy.id == policy_id).values(**data))
        await self.db.commit()
...
class OTPRepository:
    ...
    async def create(self, case_id: str, customer_id: str, otp_hash: str, expiry_minutes: int = 10) -> OTPRecord:
        otp = OTPRecord(
            id=str(uuid.uuid4()),
            case_id=case_id,
    ...
    async def mark_verified(self, otp_id: str):
        await self.db.execute(
            update(OTPRecord).where(OTPRecord.id == otp_id).values(
                status=OTPStatus.VERIFIED, verified_at=datetime.utcnow()
            )
        )
        await self.db.commit()
...
class ConsentRepository:
    ...
    async def list_all(self, skip: int = 0, limit: int = 50) -> List[ConsentRecord]:
        result = await self.db.execute(
            select(ConsentRecord).order_by(ConsentRecord.consented_at.desc()).offset(skip).limit(limit)
        )
        return result.scalars().all()
...
class EscalationRepository:
    ...
    async def list_unresolved(self) -> List[EscalationLog]:
        result = await self.db.execute(
            select(EscalationLog).where(EscalationLog.resolved == 0).order_by(EscalationLog.created_at.desc())
        )
        return result.scalars().all()

    async def list_all(self, skip: int = 0, limit: int = 50) -> List[EscalationLog]:
        result = await self.db.execute(
            select(EscalationLog).order_by(EscalationLog.created_at.desc()).offset(skip).limit(limit)
        )
        return result.scalars().all()

    async def resolve(self, escalation_id: str):
        await self.db.execute(
            update(EscalationLog).where(EscalationLog.id == escalation_id).values(
                resolved=1, resolved_at=datetime.utcnow()
            )
        )
        await self.db.commit()
...
class NotificationRepository:
    ...
    async def mark_sent(self, notification_id: str):
        await self.db.execute(
            update(NotificationLog).where(NotificationLog.id == notification_id).values(
